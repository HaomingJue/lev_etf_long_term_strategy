# ==========================================================
# SHIFTED-GRID RESEARCH  —  SPY / SSO / UPRO strategy
#
# Hypothesis: the current SPY MA100 optimum (entry=1.02, exit=0.95×MA100)
# sits at the edge of the production grid on the exit axis. The §8
# robustness heatmap shows a bright strip only at exit=0.95 with the
# adjacent value (0.97) much darker — could be a real knife-edge OR
# a discretization artifact.
#
# This script shifts BOTH entry and exit axes for SPY only:
#   ENTRY_SIGNALS  : 1.01..1.06 -> 0.98..1.03  (includes sub-MA200 entries)
#   EXIT_SIGNALS   : 0.95..1.02 -> 0.91..0.99  (denser around 0.95)
#
# Note on entry < 1.0: this means arming when price is *below* MA200
# rather than only in confirmed uptrends. For SPY (large-cap, lower vol)
# this is a meaningful but defensible strategy-concept change. For QQQ
# we would NOT lower the entry below 1.00.
#
# All other axes (drop, buy_pct, alloc_base, alloc_x2) are UNCHANGED so
# results stay directly comparable to the production grid.
#
#   ARM / BUY  : MA200  (shifted threshold)
#   EXIT       : MA100  (shifted, denser)
#
# Outputs (in ma100_shifted/):
#   - ma100_shifted_results.csv
#   - ma100_shifted_equity.png
#   - ma100_shifted_scatter.png
# ==========================================================
...
OUT_DIR = Path(__file__).parent / "ma100_shifted"
...
END           = "2026-05-30"  # bumped to current data for the shifted-grid sweep
CAPITAL       = 10_000
DD_LIMIT      = 0.40
DD_START_YEAR = 2009

# Shifted grid: entry pulled down to include sub-MA200 values, exit denser
...
def backtest(df, entry_signal, drop_level, exit_signal,
    ...
    ma100_arr  = df["MA100"].values / first["SPY"]
    n          = len(df)
...
def plot_top_combos(df, results, n=5):
    ...
    ax.set_title("Top combos — MA100 exit, SPY / SSO / UPRO", fontsize=11)
    ax.set_ylabel("Portfolio Value ($)")
    ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, _: f"${x:,.0f}"))
    ax.legend(fontsize=7, loc="upper left")
    ax.grid(True, alpha=0.35)
    plt.tight_layout()
    plt.savefig(OUT_DIR / "ma100_shifted_equity.png", dpi=150)
    print("  Saved: ma100_shifted_equity.png")
    plt.show()
...
def plot_scatter(results):
    ...
    ax.set_ylabel("CAGR (%)")
    ax.set_title("CAGR vs Worst Annual Return — MA100 exit, SPY / SSO / UPRO")
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(OUT_DIR / "ma100_shifted_scatter.png", dpi=150)
    print("  Saved: ma100_shifted_scatter.png")
    plt.show()
...
if __name__ == "__main__":
    ...
    results.to_csv(OUT_DIR / "ma100_shifted_results.csv", index=False)
    print(f"\n  Saved: ma100_shifted_results.csv  ({len(results):,} rows)")
