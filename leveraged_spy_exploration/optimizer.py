# ==========================================================
# FULL-HISTORY OPTIMIZER  —  SPY / SSO / UPRO strategy
#
# Runs every parameter combo against the FULL SPY history once.
#
# A combo PASSES if, at the end of every calendar year from 2009,
...
# from SPY daily returns using the standard leverage model:
...
DD_START_YEAR = 2009    # only apply DD filter from this year onward
                        # (synthetic lev data before UPRO inception is too punishing)
...
ALLOC_BASES   = [0.0, 0.10, 0.20, 0.30]       # SPY fraction of portfolio
ALLOC_X2S     = [0.0, 0.25, 0.50, 0.75, 1.0]  # fraction of lev spend → SSO
...
def load_data():
    # Download SPY from 1993 inception; real lev ETFs from their launch dates.
    # SSO (2×) launched 2006-06-21, UPRO (3×) launched 2009-06-23.
    # Synthetic NAVs fill the gap before real ETFs existed.
    print("Downloading SPY, SSO, UPRO …")
    spy = download("SPY", "1993-01-01", END)
    try:
        sso  = download("SSO",  "1993-01-01", END)
    except Exception:
        sso  = pd.Series(dtype=float)
    try:
        upro = download("UPRO", "1993-01-01", END)
    except Exception:
...
def plot_top_combos(df, results, n=5):
    ...
    ax.set_title("Top combos — SPY/SSO/UPRO full history equity curves", fontsize=11)
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / f"{OUT_PREFIX}_equity.png", dpi=150)
    print(f"  Saved: {OUT_PREFIX}_equity.png")
    plt.show()
...
def plot_scatter(results):
    ...
    plt.tight_layout()
    plt.savefig(OUT_DIR / f"{OUT_PREFIX}_scatter.png", dpi=150)
    print(f"  Saved: {OUT_PREFIX}_scatter.png")
    plt.show()
...
if __name__ == "__main__":
    ...
    results.to_csv(OUT_DIR / f"{OUT_PREFIX}_results.csv", index=False)
    print(f"\n  Saved: {OUT_PREFIX}_results.csv  ({len(results):,} rows)")
    ...
    print(f"  Filter: year-end DD ≤ {DD_LIMIT*100:.0f}% from {DD_START_YEAR} onward (SPY/SSO/UPRO — synthetic pre-{DD_START_YEAR} CAGR only)")
    print("=" * 105)
    ...
    if not passing.empty:
        ...
        print("=" * 105)
        for label, p in [
            ("Baseline  entry=1.04 drop=0.01 exit=1.00 buy=0.20  0% SPY 0% SSO 100% UPRO",
             (1.04, 0.01, 1.00, 0.20, 0.0, 0.0)),
            ("Baseline2 entry=1.01 drop=0.01 exit=0.99 buy=0.10 30% SPY 25% SSO 75% UPRO",
