...
> **"buy 90% + 10% base" doesn't sum to over 100% — the lev buy is capped at `1 − base`.** Each buy first tops the base sleeve up to `alloc_base` of the portfolio (10% for Balanced), then deploys leverage — but **never more than `(1 − alloc_base)` = 90% of the total** in a single buy, and never more than the cash on hand. So Balanced holds 10% base + up to 90% TQQQ = **100% invested, never more** (the 3× lives *inside* the ETF — you don't borrow). **Why does the optimizer report "90%" and not "100%"?** Because under the cap *any* `buy_pct ≥ 1 − alloc_base` is now bit-for-bit identical — it deploys exactly all cash beyond the base regardless — so buy 90% and buy 100% produce the same result and the grid just reports the lowest tying value (which is why those duplicate combos are pruned, §3). `buy_pct` only bites when it sits *below* the cap (SPY's 20% genuinely scales in over several dips). Read Balanced as "**10% cushion + deploy the rest into 3×, capped at 90%.**" And unlike earlier versions, the base is **rebalanced to its 10% target on every trade** — topped up on each buy if it slipped, trimmed on each exit if it overshot — not a one-time fill.
