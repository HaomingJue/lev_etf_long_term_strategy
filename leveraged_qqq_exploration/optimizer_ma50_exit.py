...
def load_data():
    ...
    df = pd.DataFrame({"QQQ": qqq, "QLD": lev2_nav, "TQQQ": lev3_nav}
                      ).dropna(subset=["QQQ"])
    df = df / df.iloc[0]
...
# ----------------------------------------------------------
# DD FILTER  (identical to baseline)
# ----------------------------------------------------------
...
def check_dd(df, portfolio, qqq_nav):
    ...
    for yr in years:
        ...
        port_boy  = portfolio[yr_idxs[0]]
        port_eoy  = portfolio[yr_idxs[-1]]
...
# ----------------------------------------------------------
# GRID  (no exit<entry filter — different MAs make it meaningless)
# ----------------------------------------------------------
...
if __name__ == "__main__":
    ...
    def backtest_ma200_exit(df, entry_signal, drop_level, exit_signal,
                            buy_pct, alloc_base, alloc_x2):
        """Baseline backtest using MA200 for exit (for direct comparison)."""
        first      = df.iloc[0]
        nb_arr     = df["QQQ"].values  / first["QQQ"]
        n2_arr     = df["QLD"].values  / first["QLD"]
        n3_arr     = df["TQQQ"].values / first["TQQQ"]
        ma200_arr  = df["MA200"].values / first["QQQ"]
        n          = len(df)
        alloc_x3   = 1.0 - alloc_x2
        cash       = CAPITAL
        s_b = s_2 = s_3 = 0.0
    ...
    combos_to_compare.append((
        "MA200-balanced   entry=1.04 drop=0.010 exit=1.01 buy=0.3 QQQ=0 QLD=0 TQQQ=1",
        (1.04, 0.010, 1.01, 0.30, 0.0, 0.0)
    ))
