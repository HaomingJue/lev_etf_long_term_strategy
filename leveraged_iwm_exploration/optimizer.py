# ==========================================================
# FULL-HISTORY OPTIMIZER  —  IWM / UWM / TNA strategy
#
...
# from IWM daily returns using the standard leverage model:
...
def build_lev_nav(base: pd.Series, real: pd.Series, L: int) -> pd.Series:
    """
    Returns a NAV series (starting at 1.0 on base.index[0]) for an
...
DD_START_YEAR = 2009    # only apply DD filter from this year onward
                        # (TNA inception Nov 2008; synthetic data before is too punishing)
...
ALLOC_BASES   = [0.0, 0.10, 0.20, 0.30]       # IWM fraction of portfolio
ALLOC_X2S     = [0.0, 0.25, 0.50, 0.75, 1.0]  # fraction of lev spend → UWM
...
def load_data():
    # Download IWM from start date; real lev ETFs from their launch dates.
...
# Returns (cagr, portfolio_array, iwm_nav_array)
# ----------------------------------------------------------
...
def plot_top_combos(df, results, n=5):
    ...
    ax.set_title("Top combos — full history equity curves (IWM / UWM / TNA)", fontsize=11)
...
def plot_scatter(results):
    ...
    ax.set_ylabel("CAGR (%)")
    ax.set_title("CAGR vs Worst Annual Return — all combos (IWM / UWM / TNA)")
...
if __name__ == "__main__":
    ...
    if not passing.empty:
        ...
        print("=" * 105)
        for label, p in [
            ("Original  entry=1.04 drop=0.01 exit=1.00 buy=0.20   0% IWM  0% UWM 100% TNA",
             (1.04, 0.01, 1.00, 0.20, 0.0, 0.0)),
            ("Alt       entry=1.01 drop=0.01 exit=0.99 buy=0.10  30% IWM 25% UWM  75% TNA",
