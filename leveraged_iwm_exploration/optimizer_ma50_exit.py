# ==========================================================
# MA50-EXIT RESEARCH  —  IWM / UWM / TNA strategy
#
...
def load_data():
    ...
    df["MA50"]  = df["IWM"].rolling(50).mean()
...
def backtest(df, entry_signal, drop_level, exit_signal,
    ...
    ma50_arr   = df["MA50"].values  / first["IWM"]
    n          = len(df)
...
def plot_top_combos(df, results, n=5):
    ...
    ax.set_title("Top combos — MA50 exit, IWM / UWM / TNA", fontsize=11)
...
def plot_scatter(results):
    ...
    ax.set_ylabel("CAGR (%)")
    ax.set_title("CAGR vs Worst Annual Return — MA50 exit, IWM / UWM / TNA")
